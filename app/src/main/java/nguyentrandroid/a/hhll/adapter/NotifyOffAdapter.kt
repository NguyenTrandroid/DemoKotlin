...
class NotifyOffApdapter(val notifysOff: List<ItemNotifyDB>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val EMPTY_ITEM = 0
    private val NORMAL_ITEM = 1
    override fun getItemViewType(position: Int): Int {
        return if (notifysOff[position].source.iv104.equals(Constants.SHARE_POST) ||
            notifysOff[position].source.iv104.equals(Constants.COMMENT_POST) ||
            notifysOff[position].source.iv104.equals(Constants.LIKE_POST) ||
            notifysOff[position].source.iv104.equals(Constants.COMMENT_COMMENT) ||
            notifysOff[position].source.iv104.equals(Constants.LIKE_COMMENT)
        )
            NORMAL_ITEM
        else
            EMPTY_ITEM
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return if (viewType == NORMAL_ITEM) {
            ...
            val binding: ViewDataBinding =
                DataBindingUtil.inflate(layoutInflater, R.layout.item_notify_off, parent, false)
            ViewHolderItemNotifyOff(binding)
        } else {
    ...
    override fun getItemCount(): Int {
        return notifysOff.size
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder.itemViewType == NORMAL_ITEM) {
            val viewHolder = holder as ViewHolderItemNotifyOff
            viewHolder.bind(notifysOff[position])
        } else {
